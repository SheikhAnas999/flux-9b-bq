...
def save_calibration_batch(batch_dict: dict, save_dir: str = "calibration_data"):
    """Saves a dict of tensors as FP32 for TRT INT8 calibration."""
    os.makedirs(save_dir, exist_ok=True)
    fp32_batch = {
        name: tensor.detach().float().cpu() for name, tensor in batch_dict.items()
    }
    global file_idx
    torch.save(fp32_batch, os.path.join(save_dir, f"calib_{file_idx:04d}.pt"))
    print(f"Saved batch {file_idx} ({len(batch_dict)} tensors)")
    file_idx += 1
                ...
                with self.transformer.cache_context("cond"):
                    ...
                    if self.subprocess_config["enable_spatial_cache"]:
                        ...
                        if timestep_key not in self.spatial_cache:
                            # Read architecture dims from the loaded checkpoint's config
                            # instead of relying on SpatialCache's 4B-shaped defaults.
                            # Works for klein-4B and klein-9B alike (9B family:
                            # 8 double / 24 single blocks, 32 heads x 128 head_dim,
                            # in_channels=128 -- verify via transformer/config.json).
                            # Note: torch.compile's OptimizedModule forwards attribute
                            # access to the wrapped model, so .config resolves fine.
                            tf_config = self.transformer.config
                            self.spatial_cache[timestep_key] = SpatialCache(
                                image_seq_len=latent_model_input.shape[1],
                                output_channels=tf_config.in_channels,
                                attention_head_dim=tf_config.attention_head_dim,
                                num_attention_heads=tf_config.num_attention_heads,
                                num_layers=tf_config.num_layers,
                                num_single_layers=tf_config.num_single_layers,
                            )
